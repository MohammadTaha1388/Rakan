              ...
              <div className={`p-4 rounded-2xl border space-y-4 ${
                ...
                <div className="pt-3 border-t border-slate-700/40 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-bold flex items-center gap-2 text-amber-400">
                        ...
                        سیستم یادآور روزانه مطالعه (Daily Push Notification)
                      </div>
                      <div className="text-xs text-slate-400">
                        ارسال اعلان در ساعت مشخص برای شروع مطالعه مطابق اهداف درسی ثبت‌شده
                      </div>
                    </div>
                    <button
                      ...
                      className={`w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors ${
                        formData.dailyReminderEnabled ? 'bg-amber-500' : 'bg-slate-700'
                      }`}
                    >
                      <div
                        className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${
                          formData.dailyReminderEnabled ? 'translate-x-0' : '-translate-x-6'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  {formData.dailyReminderEnabled && (
                    <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-700/80 space-y-3 animate-in fade-in duration-150">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <input
                          type="time"
                          value={formData.dailyReminderTime || '17:00'}
                          onChange={e =>
                            setFormData(prev => ({
                              ...prev,
                          ...
                          className="px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-700 text-slate-200 text-xs font-mono focus:outline-none focus:border-amber-500 text-center"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5 text-xs">
                          {[
                            ...
                            const activeDays = formData.dailyReminderDays || [0, 1, 2, 3, 4, 5, 6];
                            const isIncluded = activeDays.includes(d.day);
                            return (
                              <button
                                key={d.day}
                                type="button"
                      ...
                      <div className="flex items-center justify-between pt-1 text-[11px]">
                        <span className="text-slate-400">
                          هدف مطالعاتی: <strong>{formData.studyGoal || 'مطالعه روزانه دروس'}</strong>
                        </span>
                        <button
                          type="button"
                          onClick={() => {
                            dispatchAppNotification(
                              '⏰ یادآور مطالعه مدرسه راکان',
                              `زمان شروع مطالعه فرارسید! هدف امروز: ${formData.studyGoal || 'مرور مباحث و حل تست'}. مشاور همراه شماست.`,
                              'dailyReminder',
                              true
                            );
                          }}
